...
## Endpoint map (PARTIAL â€” from `src/app.ts` + route files)

> âš ï¸ This map documents roughly a third of the mounted route groups. `src/app.ts` mounts ~28
...
> **Absence from this file does not mean an endpoint doesn't exist** â€” check `src/app.ts` and the
> matching `*.routes.ts`.
...
| POST | `/assistant/command` | Agentic command â†’ answer/action. Supports `{ message, conversationId?, attachmentBase64?, attachmentMimeType?, attachmentName?, clientNow?, tz? }` |
...
### `/api/v1/professional` â€” real-estate listing photos & public pages
...
| POST | `/listings/:id/photos` | Upload one image â€” `{ base64, contentType? }`; `base64` may be a bare payload or a `data:image/â€¦;base64,â€¦` URL. Max 5 MB, jpeg/png/webp, 12 per listing â†’ `{ photos, shareUrl }` |
| DELETE | `/listings/:id/photos` | `{ url }` â€” drop a photo and delete the stored object |
| POST | `/listings/:id/publish` | Idempotent: create (or return) the public page â†’ `{ shareUrl, slug }` |
...
curl -s -X POST "$API/api/v1/professional/listings/$LID/publish" -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" -d '{}'   # â†’ { "shareUrl": "https://â€¦/l/<slug>", "slug": "â€¦" }
curl -s "$API/l/<slug>"                          # no auth
```

The agent tool `send_listing_to_buyer` (`POST /professional/action`) chains publish + Gmail:
`{"name":"send_listing_to_buyer","args":{"name":"<lead>","address":"<listing>","note":"â€¦"}}`.
...
### `/api/v1/composio` (brokered integrations â€” HubSpot, Stripe, Linear, Zoom, â€¦)
...
- `GET /toolkits` â†’ `{ available, toolkits: [{ slug, name, description, audience, status }] }`.
  `available: false` means the server has no Composio key. `status` is
  `disconnected|pending|active|failed|revoked`.
- `POST /connect` `{ toolkit }` â†’ `{ redirectUrl }` â€” the Composio-hosted consent URL to open.
  `503` when Composio isn't configured; `400` for an unknown toolkit slug.
- `GET /connections` â†’ `{ connections }` â€” **reconciles against Composio**, so this is the call that
  flips a connection `pending` â†’ `active`. The app polls it after the browser consent step.
- `DELETE /connections/:toolkit` â†’ `{ ok: true }` â€” revokes upstream at Composio, then marks the row
  revoked locally.
- `GET /callback` â€” public (no auth), no code exchange; just deep-links the browser back into the app.
...
The authenticated lead workspace lists marketing and inbound contacts, shows a transparent 0â€“100 prioritization score with its contributing reasons, and lets the marketer control qualification status. The score is a sorting aid, not a conversion prediction.
...
| POST | `/api/v1/notifications/:id/send-reply` | Body `{ body }` â€” the exact text the user approved. Sends via Gmail from the thread's own account and resolves the item. Never regenerates the text server-side. |
