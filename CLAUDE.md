...
cURL tests. (The old `mvp-*.md` / `GEMINI.md` docs were removed â€” they described a Flutter
client and a Redis/BullMQ stack that no longer exist.)

## Commands

```bash
npm run dev          # tsx watch src/server.ts â€” local dev server (port 5000)
npm run build        # tsc â†’ dist/
npm start            # node dist/server.js â€” run compiled build
npm run lint         # tsc --noEmit â€” typecheck only; there is no ESLint here
npm run migrate      # tsx src/db/migrations/runner.ts â€” apply SQL migrations in order
...
npm test             # tsx --test â€” explicit 23-file suite (95 tests as of 2026-09-24)
```
...
`package.json` â€” that is the single easiest way to write a test that silently never runs.
...
- [src/app.ts](src/app.ts) builds the Express app and **also calls `initKeyring()` itself** â€”
...
### Layers
Routes (`src/routes/*.routes.ts`) â†’ Services (`src/services/**`) â†’ DB (`src/config/db.ts`).
There is no controllers layer â€” route files are thin and call services directly. The `query()`
helper in [src/config/db.ts](src/config/db.ts) runs parameterized SQL against a `pg.Pool`
(SSL enabled for Supabase). All data access is **raw SQL** â€” no ORM.
...
Plain numbered SQL files in [src/db/migrations/](src/db/migrations/) (`001_*.sql` â€¦ `091_*.sql`
as of 2026-09-24) applied in filename order by [runner.ts](src/db/migrations/runner.ts) via
`npm run migrate`.
**To change schema, add the next numbered migration file â€” never edit an applied one.** Note
...
3. Response code is the retry contract:
   - **200** â†’ delivered/done.
   - **422** (`PermanentJobError` from [src/jobs/errors.ts](src/jobs/errors.ts)) â†’ QStash does NOT retry.
   - **5xx** â†’ transient; QStash retries with backoff (default 5 retries).
...
**Multi-account (Personal âŸ· Work):** a user can connect several Google accounts. `google_accounts`
is keyed by `id` (not `user_id` â€” the old `UNIQUE(user_id)` is gone) and carries `email` + `role`
...
`resolveGoogleAccount(userId, mode)` (role â†’ primary â†’ most-recent), account-scoped
...
### Email
Two distinct paths: **Gmail API** for decline-email sends (the product feature â€”
...
### AI
**Two systems live under `src/services/ai/` â€” know which one your path uses.**

1. **`geminiClient.ts` â€” the agent brain, used by BOTH modes.** Gemini REST (no SDK) with
...
2. **`provider.ts` (`getProvider({ mode })`) â€” the older JSON-text abstraction**, used at only
   three call sites in `ai.service.ts`. Professional mode â†’ Gemini. Personal mode â†’
   ...
   template fallback, so a missing `AI_API_KEY` degrades silently rather than erroring â€” check
...
One cross-app queue of things **waiting on the user** â€” the bell, the "Waiting on you" widget,
...
- **Tables** (migration `062`): `notification_items` (the feed), `notification_source_cursors`
  ...
  staleness warning shown *in the widget* â€” a hub that has silently stopped ingesting is worse
...
- **Inclusion bar:** *if the user does nothing, does something bad or slow happen?* If no, it is
  activity, not a notification. Do not relax this â€” a mirror of nine apps' firehoses is the hassle
...
- **Dedup keys are provider-scoped, not adapter-scoped** â€” native and Composio delivery of one
  Gmail message both write `gmail:<threadId>` and collapse into a single row. Where a source table
  itself contains duplicates (it happens â€” `advisor_compliance_items` had 30 rows for 4 distinct
...
- **Retention is state-based, not age-based** (`hubRetention.service.ts`): resolved/dismissed
  ...
  meant to keep holding â€” age alone was the wrong rule.
...
- **Fan-out rule:** external adapters must enqueue **one QStash job per source per user who
  ...
  credential is `expired`/`reauth_required` ARE still polled â€” the adapter is what writes
...
- **Every adapter must RECORD a credential failure, never throw it.** Look up and decrypt the
  ...
  `notification_source_health` unwritten â€” the source stalls while the user sees a clean feed,
...
- **Sources.** Local adapters (`internal`, `calendar`) read only our own tables and run inline for
  ...
  credential-holding user per source onto the `hub` queue â†’ `hub.processor.ts`. Composio webhooks write to the hub too
  (`composioTriggers` no longer pushes â€” same event, same behaviour, whichever way the account was
  connected).
  ...
  - **GitHub** uses `GET /notifications` and filters on the `reason` field â€” only
  ...
  - **Todoist** includes only tasks due **today or earlier**; an undated task is a personal
    ...
    Todoist returns `2026-08-31T09:00:00` for timed tasks â€” compare the date part, or a task due
  ...
  - **Listing views** (`re_listing_views`, migration `063`) are the Real Estate persona's own
    signal â€” Composio has no Zillow/AppFolio toolkit, but we host the page. Coarse by design:
...
- **The daily summary** (`hubSummary.service.ts`) is generated once per user per day, cached in
  `ai_outputs`, and shared with the digest. The model **narrates a computed tally** â€” it is handed
...
- **Active users** for the scheduler are those with a connected account or professional data â€”
...
- **Inline reply** (`hubReply.service.ts`, `POST /:id/draft-reply` + `POST /:id/send-reply`) is
  ...
  client passes back â€” it never regenerates. One-tap send of model-written text to a real
...
- **Composio alert triggers are verified, and most toolkits cannot be wired.** `quickbooks`,
  ...
  `DELIBERATELY_UNWIRED_TOOLKITS` in `composioTriggers.service.ts` â€” a toolkit having *tools*
...
`hub-retention` daily (`45 3 * * *`), both against `https://interlink-backend.vercel.app` â€”
...
### Composio â€” the brokered long tail of integrations
...
Zoom, Calendly, Dropbox, Airtable, Telegram, Discord, Canvas â€” Composio owns the OAuth apps
...
than a Composio-managed one â€” **Canvas** (`CANVAS_CLIENT_ID/SECRET`). `getOrCreateAuthConfig()`
creates a custom-auth config from those env vars (`BYOC_CREDENTIALS` map); unset â†’ the toolkit
...
which rides the shared Google OAuth `youtube` scope â€” search + playlists work; the app opens a
...
- **Schema sanitizer.** `toGeminiSchema()` is load-bearing: Composio emits full JSON Schema and
  ...
  entire turn â€” including every native tool in the same request.
...
- **Read-only defaults to WRITE.** Only `GET_/LIST_/SEARCH_/â€¦` verbs auto-chain; anything else goes
  through confirm-before-execute. Deliberate â€” auto-running an unknown `STRIPE_CREATE_REFUND` is not
...
### Professional Mode â€” Financial Advisor (finance persona)
The `finance` persona is branded **Financial Advisor** (label/copy only â€” the persona key, routes, and
...
### Professional verticals â€” external data
...
**Real Estate** `search_market` prefers RentCast (`RENTCAST_API_KEY`) â†’ RapidAPI Realtor
(`RAPIDAPI_KEY`) â†’ **SimplyRETS** ([simplyrets.service.ts](src/services/professional/realestate/simplyrets.service.ts)),
the keyless default that returns realistic demo MLS listings (Houston, TX) with no setup â€” so listing
...
licensing + MLS membership â€” no API key gets past that â€” so listings are marketed from our own
infrastructure via
...
outside `/api/v1`). The slug is random rather than the listing id because the page is public â€” ids
...
resolve `PUBLIC_BASE_URL` â†’ Vercel's `VERCEL_PROJECT_PRODUCTION_URL` â†’ `API_BASE_URL`, deliberately
