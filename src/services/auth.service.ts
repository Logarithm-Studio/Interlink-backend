...
export async function storeTokens(
  ...
  // We reuse the same IV/tag/kid for both columns since they are encrypted
  // with independent random IVs — each encrypt() call generates a fresh IV.
  await query(
    ...
    [
      ...
      provider,
      tokens.expiresAt,
      // Pack both encrypted blobs: "accessEnc|refreshEnc" separated by column
      // Actually store them individually — access in access_token_enc, refresh
      // in refresh_token_enc.  The IV/tag/kid columns store access-token params;
      // refresh token uses a separate set encoded as "iv:tag:kid" prefix in
      // the refresh_token_enc column itself for compactness.
      encAccess.ciphertext,
      `${encRefresh.iv}:${encRefresh.tag}:${encRefresh.kid}:${encRefresh.ciphertext}`,
      encAccess.iv,
      encAccess.tag,
      encAccess.kid,
    ],
...
export async function getTokens(
  ...
  try {
    const parts = (row.refresh_token_enc ?? "").split(":");
    if (parts.length < 4) throw new Error("invalid format");
    const [rIv, rTag, rKid, ...rest] = parts;
    refreshToken = decrypt(rest.join(":"), rIv, rTag, rKid);
  } catch {
    await markReauthRequired(userId, provider);
    throw new ReauthRequiredError(userId, provider);
  }
